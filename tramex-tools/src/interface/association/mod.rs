...
pub use relation::{AssociationStatus, TraceRelation};
pub use rules::{AssociationRule, AssociationRules, NasToRrcRule, SearchDirection};
...
pub fn compute_associations(events: &mut [Trace], rules: &AssociationRules, start_index: usize) {
    ...
    for index in start_index..trace_count {
        ...
        if in_lookback &&
             let Some(trace) = events.get_mut(index) &&
                // Only reset if it was NotFound (might find match in new batch)
                matches!(trace.relation.parent, AssociationStatus::NotFound)
        {
        ...
        for rule in applicable_rules {
            let status = TraceMatcher::find_relative(index, events, rule, &layer, &rule.target_layer());
