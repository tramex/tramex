...
fn test_nas_rrc_associations() {
    ...
    assert!(data.events.len() == NB_EVENTS, "Total traces should be {}, found {}", NB_EVENTS, data.events.len());
    ...
    assert!(data.events[0].relation.get_child_indices() == Some(&vec![1]), "Child index of trace 0 should be 1");
    assert!(data.events[1].relation.get_parent_indices() == Some(&vec![0]), "Parent index of trace 1 should be 0");
    ...
    assert!(data.events[14].relation.get_child_indices() == Some(&vec![15]), "Child index of trace 14 should be 15");
    assert!(data.events[15].relation.get_parent_indices() == Some(&vec![14, 16]), "Parent index of trace 15 should be 14 and 16");
    assert!(data.events[16].relation.get_child_indices() == Some(&vec![15]), "Child index of trace 16 should be 15");
    
    assert!(data.events[17].relation.get_child_indices() == Some(&vec![18]), "Child index of trace 17 should be 18");
    assert!(data.events[18].relation.get_parent_indices() == Some(&vec![17, 19]), "Parent index of trace 18 should be 17 and 19");
    assert!(data.events[19].relation.get_child_indices() == Some(&vec![18]), "Child index of trace 19 should be 18");
}
