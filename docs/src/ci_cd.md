...
This file is executed when a push command is performed on the main branch of the Git repository. It can be ran automatically when trying to push or manually from the Action tab. It is executed when a user tries to launch the web application of Tramex.
...
The website is built using the `trunk` command. It is import not to forget the `release` flag in order to optimize the performances and the size of the output code. It is also necessary to use the `public-url` flag because Tramex is stored on a private domain. To ease the deployment, all the built files are stored in the `dist` directory.
...
In the Tramex application, it is possible to open documentation from the About section. This documentation appears as a website and is generated from the `Mdbook`. `Mdbook` is static site generator that publishes a website using `Markdown` files. The `Mdbook` is also set up and built in the `dist` directory.
