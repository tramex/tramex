...
use super::{DataSource, EventBus, EventContext, EventStore, EventSubscriber};
...
impl Application {
    ...
    pub fn navigate_next(&mut self) -> bool {
        ...
        log::debug!(
            "navigate_next: starting from index {}, total events loaded: {}",
            start_index,
            total_events
        );
        ...
        loop {
            ...
            if !can_continue {
                log::debug!(
                    "Application: Reached end of events at index {}",
    ...
    pub fn is_fully_loaded(&self) -> bool {
        self.data_source.as_ref().map(|s| !s.has_more()).unwrap_or(true)
    }
    ...
    pub fn is_auto_loading(&self) -> bool {
        self.data_source.as_ref().map(|s| s.is_auto_loading()).unwrap_or(false)
    }

    /// Check if data source has more data available
    pub fn has_more_data(&self) -> bool {
        self.data_source.as_ref().map(|s| s.has_more()).unwrap_or(false)
    }
