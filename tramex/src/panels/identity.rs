...
use tramex_tools::{
    ...
    interface::{
        layer::Layer,
        parse_config::{FileMetadata, Technology},
    },
};
...
impl Identity {
    ...
    fn decode_tai_list(data: &str) -> TaiInfo {
        ...
        match list_type {
            ...
            0b01 => {
                ...
                let mut i = tac_start;
                while i + 3 <= bytes.len() {
                    let tac = format!("{:02X}{:02X}{:02X}", bytes[i], bytes[i + 1], bytes[i + 2]);
                    tacs.push(tac);
                    i += 3;
                }
            }
    ...
    fn parse_registration_accept(&mut self, text: &[String]) {
        ...
        for line in text {
            ...
            if !trimmed.is_empty()
                && !trimmed.starts_with(' ')
                && trimmed.contains(':')
                && !trimmed.starts_with("5G-GUTI:")
                && !trimmed.starts_with("TAI list:")
            {
                in_guti = false;
                in_tai = false;
            }
            ...
            // Parse TAI list
            if in_tai && let Some(value) = Self::extract_field(trimmed, "Data =") {
    ...
    fn parse_pdu_session_accept(&mut self, text: &[String]) {
        ...
        for line in text {
            ...
            if in_qos_flow && let Some(value) = Self::extract_field(trimmed, "5QI =") {
    ...
    fn render_field(ui: &mut egui::Ui, label: &str, value: &Option<String>) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(RichText::new(value.as_ref().map(|s| s.as_str()).unwrap_or("N/A")).color(theme.text));
        });
    }
    ...
    fn ui(&mut self, ui: &mut egui::Ui) {
        egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
            ...
            match self.metadata.technology {
                    ...
                    ui.group(|ui| {
                        ...
                        if self.nr_qos_rules.is_empty() {
                            ...
                            for (i, rule) in self.nr_qos_rules.iter().enumerate() {
                                ui.add_space(3.0);
                                ui.label(RichText::new(format!("Rule {}:", i + 1)).color(theme.text_weak).italics());
                ...
                Technology::Unknown => {
                    ui.label(RichText::new("No NAS identity data available").color(theme.text_weak));
                }
            }
        });
    }
